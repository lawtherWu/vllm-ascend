...
* `recompute`: When KV loading fails, vLLM rolls the request back to the last valid prefix and reschedules it to recompute the failed KV blocks. For hybrid attention models (e.g. DeepSeekV4, Qwen 3.5), block-level rollback is not available. On PD-disaggregated Decode nodes, enable `recompute_scheduler_enable` so the complete request is returned to the load-balancing proxy for retry.
