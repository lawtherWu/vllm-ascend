...
class NPUPlatform(Platform):
    ...
    @staticmethod
    def _validate_layerwise_host_offload_config(vllm_config: VllmConfig) -> None:
        """Validate the frozen first-release DSA offload shape."""
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        cls._validate_kv_load_failure_policy(
            vllm_config,
            recompute_scheduler_enabled=ascend_config.recompute_scheduler_enable,
        )
    ...
    def _validate_kv_load_failure_policy(
        vllm_config: VllmConfig,
        recompute_scheduler_enabled: bool = False,
    ) -> None:
        kv_transfer_config = vllm_config.kv_transfer_config
        if kv_transfer_config is None:
            return
        if getattr(kv_transfer_config, "kv_load_failure_policy", "fail") == "recompute":
            assert not getattr(vllm_config.model_config, "is_hybrid", False) or recompute_scheduler_enabled, (
                "Hybrid models require recompute_scheduler_enable=true on a PD-disaggregated "
                "Decode node to use recompute mode KV load failure policy."
            )
